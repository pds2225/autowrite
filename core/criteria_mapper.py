...
_RULES: list[tuple[str, str, str, float]] = [
    ...
    # ── MARKET — 시장분석 ───────────────────────────────────────
    (r"시장\s*분석|목표\s*시장|tam|sam|som|시장\s*규모|경쟁\s*력",MARKET,  "시장분석/TAM-SAM-SOM", 1.0),
    (r"경쟁\s*력\s*확보|시장\s*진입",                              MARKET,  "경쟁력/시장진입",      1.0),
    (r"2\s*[-–]\s*2|2\.2",                                         MARKET,  "섹션번호 2-2",         0.9),
    (r"1\s*[-–]\s*2|1\.2",                                         MARKET,  "섹션번호 1-2",         0.9),

    # ── SOLUTION — 솔루션/아이템 ────────────────────────────────
    # 키워드 우선: 필요성·기술완성도·차별성은 섹션번호보다 강하게
    (r"필요성\s*및\s*기술|기술\s*완성도|기술\s*확보|기술적.*차별",SOLUTION, "기술완성도/차별성",    1.0),
    (r"차별성|기능적\s*차별|기술적\s*차별",                         SOLUTION, "기술/기능 차별성",     1.0),
    (r"솔루션|기술|서비스|아이템|제품|준비\s*현황|개발\s*현황",    SOLUTION, "솔루션/아이템",        1.0),
    (r"구체화|실현|고도화",                                         SOLUTION, "구체화/고도화",        0.85),
    (r"1\s*[-–]\s*1|1\.1",                                         SOLUTION, "섹션번호 1-1(솔루션)", 0.85),  # 1-1이 기술관련일 때
    (r"2\s*[-–]\s*1|2\.1",                                         SOLUTION, "섹션번호 2-1",         0.9),

    # ── BM — 비즈니스모델 ───────────────────────────────────────
    (r"비즈니스\s*모델|수익\s*구조|수익\s*모델|bm|가격|pricing",   BM,      "BM/수익구조",          1.0),
    # 주의: 3-1이 BM인 서식도 있고 SCALEUP인 서식도 있음 → 키워드 우선
    (r"3\s*[-–]\s*1|3\.1",                                         BM,      "섹션번호 3-1",         0.8),  # 신뢰도 낮춤

    # ── SCALEUP — 성장전략/자금 ─────────────────────────────────
    # 키워드 우선: 중장기 성장 계획은 SCALEUP (BM 섹션번호보다 높은 신뢰도)
    (r"중장기\s*성장|장기\s*계획|성장\s*계획",                      SCALEUP, "중장기성장계획",       1.0),
    (r"성장\s*전략|사업화\s*전략|스케일\s*업|scale|추진\s*일정|로드맵", SCALEUP, "성장전략/로드맵",  1.0),
    (r"자금|예산|사업비|정부\s*지원|집행",                          SCALEUP, "자금/예산",            0.95),
    (r"정부지원사업\s*선정|지원사업\s*선정",                        SCALEUP, "정부지원선정",         0.9),
    (r"3\s*[-–]\s*[23]|3\s*[-–]\s*3|3\.[23]",                     SCALEUP, "섹션번호 3-2/3-3",     0.9),

    # ── TEAM — 팀구성 ───────────────────────────────────────────
    (r"팀|조직|대표|구성원|인력|역량|경력|학력",                    TEAM,    "팀/조직구성",          1.0),
    (r"인프라|보유\s*자원|활용\s*계획",                             TEAM,    "인프라/보유자원",      0.9),
    (r"4\s*[-–]\s*[1-5]|4\.[1-5]",                                 TEAM,    "섹션번호 4-x",         0.85),
]
