...
class OpenAIService:
    DEFAULT_ANTHROPIC_WRITING_MODEL = "claude-opus-4-1-20250805"

    def __init__(self, settings: Settings):
        self.settings = settings
        self.provider = settings.ai_provider
        self._client = OpenAI(api_key=settings.openai_api_key) if settings.has_openai else None
        self._runtime_disabled_reasons: dict[str, str] = {}
        self._anthropic_writing_model = (
            os.getenv("AUTO_WRITE_ANTHROPIC_WRITING_MODEL", "").strip() or self.DEFAULT_ANTHROPIC_WRITING_MODEL
        )

    @property
    def available(self) -> bool:
        return self.provider_available("openai") or self.provider_available("anthropic")

    @property
    def anthropic_writing_model(self) -> str:
        return self._anthropic_writing_model

    @property
    def status_text(self) -> str:
        available_providers: list[str] = []
        if self.provider_available("openai"):
            available_providers.append("OpenAI")
        if self.provider_available("anthropic"):
            available_providers.append("Claude")
        if available_providers:
            if len(available_providers) == 2:
                return "OpenAI, Claude 사용 가능"
            if available_providers[0] == "Claude":
                return "Claude 사용 가능 (이미지는 기본 카드로 대체될 수 있음)"
            return f"{available_providers[0]} 사용 가능"
        if self._runtime_disabled_reasons:
            joined = "; ".join(
                f"{provider}: {reason}" for provider, reason in sorted(self._runtime_disabled_reasons.items())
            )
            return f"AI 연결 비활성화: {joined}"
        return "AI API 키 없음, 기본 동작만 사용"

    def provider_available(self, provider: str) -> bool:
        if provider == "openai":
            return self.settings.has_openai and provider not in self._runtime_disabled_reasons
        if provider == "anthropic":
            return self.settings.has_anthropic and provider not in self._runtime_disabled_reasons
        return False

    def _resolve_provider(self, provider_override: str | None = None) -> str:
        if provider_override:
            return provider_override
        if self.provider_available(self.provider):
            return self.provider
        if self.provider != "openai" and self.provider_available("openai"):
            return "openai"
        if self.provider != "anthropic" and self.provider_available("anthropic"):
            return "anthropic"
        return self.provider
    ...
    def _disable_runtime(self, provider: str, reason: str) -> None:
        if self._runtime_disabled_reasons.get(provider):
            return
        self._runtime_disabled_reasons[provider] = reason
        provider_name = "OpenAI" if provider == "openai" else "Anthropic"
        log_line(f"[WARN] {provider_name} runtime disabled: {reason}")
    ...
    def complete_json(
        ...
        if not self.provider_available(provider):
            return None
        output_text = self._complete_text(
            system_prompt,
            user_prompt,
            model=model,
            temperature=0.2,
    ...
    def _complete_text_openai(
        ...
        try:
            ...
            if self._is_auth_error(exc):
                self._disable_runtime("openai", "API 키 인증 실패")
    ...
    def _complete_text_anthropic(
        ...
        try:
            ...
            if self._is_auth_error(exc):
                self._disable_runtime("anthropic", "API 키 인증 실패")
    ...
    def draft_missing_answers(
        ...
        if strict_preserve:
            system_prompt = (
                ...
                "\n"
                "[절대 원칙 - 반드시 준수]\n"
                "1. 참고자료(원문)에 있는 내용만 사용하세요. 원문에 없는 내용을 새로 만들지 마세요.\n"
                "2. 숫자, 금액, 날짜, 기업명, 사람 이름, 고유명사는 원문 그대로 유지하세요.\n"
                "3. 줄글 원문 → 표 칸 항목: 핵심 내용을 요약하되 수치·고유명사는 그대로 유지\n"
                "4. 표 칸 원문 → 줄글 항목: 원문의 내용을 자연스럽게 연결하되 내용 추가 금지\n"
                "5. 항목명 표현이 달라도(경력→이력, 추진→수행 등) 내용은 원문 기준으로 작성\n"
                "6. 원문에서 해당 항목에 맞는 내용을 찾지 못하면 빈 문자열(\"\")로 반환\n"
                "\n"
                "Return a JSON object where each key is question_id and each value is Korean text from the source material."
            )
        else:
            system_prompt = (
        ...
        if (
            ...
            and model_override
            and model_override != self.settings.anthropic_model
            and self.provider_available("anthropic")
        ):
            log_line(f"[WARN] Anthropic 작성 모델({model_override}) 실패, 기본 모델로 재시도합니다.")
    ...
    def summarize_sources(self, topic: str, snippets: list[dict[str, str]]) -> str:
        ...
        model = self.settings.openai_search_model if provider == "openai" else self.settings.anthropic_search_model
        ...
        if isinstance(payload, dict):
            ...
            if isinstance(sources, list):
                ...
                for item in sources:
                    ...
                    cleaned.append({"title": title, "url": url, "organization": organization, "snippet": snippet})
                return cleaned
        return []
